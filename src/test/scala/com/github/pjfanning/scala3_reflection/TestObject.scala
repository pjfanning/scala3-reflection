...
class TestObject extends munit.FunSuite:
  ...
  test("reflect basic Generic class declared inside Object") {
    ...
    assertEquals( result.show(), """ScalaCaseClassInfo(com.github.pjfanning.scala3_reflection.TestObject$GenericTestClass):
                                   |   fields:
                                   |      (0)[T] t: scala.Int
                                   |""".stripMargin)
    assertEquals(result.infoClass.getName, "com.github.pjfanning.scala3_reflection.TestObject$GenericTestClass")
  }
